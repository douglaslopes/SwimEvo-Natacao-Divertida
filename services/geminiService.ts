...
export const generateSwimFeedback = async (
  ...
  if (!ai) {
    return "Não foi possível conectar ao treinador IA (API Key ausente).";
  }
...
2. Se o relato for negativo, seja empático e faça uma piada leve com o nível atual.
3. Dê uma dica técnica simples focada em mindfulness ou cabeça conectada na água.
...
export const generateWeeklyChallenge = async (
  ...
  const prompt = `
Crie um mini-desafio de natação divertido para alguém no nível "${levelTitle}".
Foco em técnica ou respiração.
Máximo 15 palavras.
`;
